...
from utils.datasets_gan_yolo import create_dataloader_rgb_ir
...
def run(data,
    ...
    if training:  # called by train.py
        ...
        gan_model.half() if half else gan_model.float()
    else:  # called directly
        device_ids = [int(id_) for id_ in device.split(',')]
        ...
        if isinstance(gan_model, torch.nn.DataParallel):
            gan_model.module.load_state_dict(state_dict_netG)
        else:
            gan_model.load_state_dict(state_dict_netG)
            gan_model = nn.DataParallel(gan_model, device_ids=device_ids) 
    ...
    if not training:
        ...
        test_path_ir = data[task + '_ir']
        dataloader = create_dataloader_rgb_ir(test_path_rgb, test_path_ir, imgsz, batch_size, stride, single_cls, pad=pad, rect=pt,
    ...
    for batch_i, (imgs_rgb, imgs_ir, targets, paths, shapes) in enumerate(pbar):
        t1 = time_sync()
        if pt or jit or engine:
            img_rgb = imgs_rgb.to(device, non_blocking=True)
            img_ir = imgs_ir.to(device, non_blocking=True)
        ...
        # Inference
        gan_img = gan_model(img_rgb)
        loss_L1 = criterionL1(gan_img, img_ir)
        loss_L2 = criterionL2(gan_img, img_ir)
        ...
        # Loss
        if compute_loss:
            loss_f = compute_loss([x.float() for x in train_out], targets)[1]  # box, obj, cls
            loss_g = torch.tensor([loss_L1, loss_L2], device=device)
            loss_all = torch.cat((loss_f, loss_g))
            loss += loss_all
        ...
        for si, pred in enumerate(out):
            ...
            if single_cls:
                pred[:, 5] = 0
            predn = pred.clone()
            scale_coords(gan_img_resize[si].shape[1:], predn[:, :4], shape, shapes[si][1])  # native-space pred

            # Evaluate
            if nl:
                tbox = xywh2xyxy(labels[:, 1:5])  # target boxes
                scale_coords(gan_img_resize[si].shape[1:], tbox, shape, shapes[si][1])  # native-space labels
            ...
            callbacks.run('on_val_image_end', pred, predn, path, names, gan_img_resize[si])
