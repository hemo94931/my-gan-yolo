...
from utils.general import (DATASETS_DIR, LOGGER, NUM_THREADS, check_dataset, check_requirements, check_yaml, clean_str,
                           segments2boxes, xyn2xy, xywh2xyxy, xywhn2xyxy, xyxy2xywhn, xyxy2xywh)
...
#         logging.info(f'{prefix}New cache created: {path}')
    ...
    def __getitem__(self, index):
        ...
        img_rgb = img_rgb[:, :, ::-1]  # BGR to RGB, to 3x416x416
        img_rgb = np.ascontiguousarray(img_rgb)
        img_ir = img_ir[:, :, ::-1]  # BGR to RGB, to 3x416x416
        img_ir = np.ascontiguousarray(img_ir)
        transform_list = [transforms.ToTensor()]
        transform_list += [transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)), transforms.Resize([512,512])]
        # transform_list += [transforms.Resize([512,512])]
        trans = transforms.Compose(transform_list)
        img_ir = trans(img_ir)
        img_rgb = trans(img_rgb)
#         img_all = torch.cat((img_rgb, img_ir),0)
#         img_all = np.concatenate((img_rgb, img_ir), axis=0)

#         return torch.from_numpy(img_all), labels_out, self.img_files_rgb[index], shapes
        return img_rgb, img_ir,labels_out, self.img_files_rgb[index], shapes
        # return torch.from_numpy(img_rgb), torch.from_numpy(img_ir),labels_out, self.img_files_rgb[index], shapes
